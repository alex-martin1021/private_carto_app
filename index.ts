...
// const cartoMapId_test = import.meta.env.VITE_MAP_ID;
// const apiBaseUrl_test = import.meta.env.VITE_API_BASE_URL; 
// const clientId_test = import.meta.env.VITE_CLIENT_ID;

// const auth0Client = await createAuth0Client({
//   domain: apiBaseUrl_test,
//   clientId: clientId_test,
// });

// const accessToken_test = await auth0Client.getTokenSilently();

// fetchMap({
//   cartoMapId: cartoMapId_test,
//   apiBaseUrl: apiBaseUrl_test,
//   clientId: clientId_test,
//   accessToken: accessToken_test
// }).then(cartoMap => deck.setProps({
...
const MAP_CONFIGS = [
  {
    // Replace with your private map ID
    id: "838ff113-1b19-4b86-8712-106550b2fb06",
    name: "Please for the love of god work",
  },
];
...
async function initialize(selectedMapIds: string[]) {
  try {
    ...
    allFetchedMapData.forEach((mapData, index) => {
      ...
      if (mapData.popupSettings && mapData.popupSettings.layers) {
        for (const layerId in mapData.popupSettings.layers) {
          ...
          mergedPopupSettingsLayers[layerId] =
            mapData.popupSettings.layers[layerId];
        }
      }
      ...
      if (index === 0) {
        ...
        if (mapData.initialViewState) {
          finalInitialViewState = {
            ...INITIAL_VIEW_STATE,
            ...mapData.initialViewState,
          };
        } else if (
          (mapData as any).mapOptions &&
          (mapData as any).mapOptions.viewState
        ) {
          finalInitialViewState = {
    ...
    if (currentMapData.layers && currentMapData.layers.length > 0) {
      ...
      legendElement.addEventListener(
...
function setupMapSelector() {
  ...
  const firstCheckbox = mapSelectorContainer.querySelector<HTMLInputElement>(
    'input[type="checkbox"]'
  );
  if (firstCheckbox) {
